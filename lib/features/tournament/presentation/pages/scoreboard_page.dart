...
class _ScoreboardPageState extends ScoreboardStateBase<ScoreboardPage> {
  // ── TTS 직접 구현 ─────────────────────────────────────────────
  final FlutterTts _pageTts = FlutterTts();
  bool _ttsOn = true;

  Future<void> _ttsInit() async {
    await _pageTts.setLanguage('ko-KR');
    await _pageTts.setSpeechRate(0.9);
    await _pageTts.setVolume(1.0);
    await _pageTts.setPitch(1.0);
  }
  ...
  // 코트체인지 기준 합계: 21점→11, 25점→13
  int get _cpTotal => targetScore == 21
      ? 11
      : targetScore == 25
      ? 13
      : (targetScore / 2).ceil();
            ...
            child: Align(
              ...
              child: ConstrainedBox(
                constraints: BoxConstraints(
                  maxWidth: media.size.width * maxWidthFactor,
