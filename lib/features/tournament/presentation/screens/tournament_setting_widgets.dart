          ...
          children: [
            Text(label,
                style: TextStyle(
                    ...
                    color: selected ? const Color(0xFF5B8ABB) : const Color(0xFF888888))),
            if (badge.isNotEmpty)
              Text(badge,
                  style: TextStyle(
                      fontSize: 10,
                      color: selected ? const Color(0xFF5B8ABB) : const Color(0xFFAAAAAA))),
          ],
        ),
      ),
    ),
...
class SettingSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Row(children: [
        Icon(icon, size: 18, color: const Color(0xFF5B8ABB)),
...
class CourtSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final rec = selectedCount > 0 ? (selectedCount / 4).floor().clamp(1, 10) : 0;
    return Container(
            ...
            children: List.generate(10, (i) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  width: 56, height: 52,
                  decoration: BoxDecoration(
                    color: isSel ? const Color(0xFF5B8ABB)
                        : isRec ? const Color(0xFFEEF4FB) : const Color(0xFFF5F7FA),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                      color: isSel ? const Color(0xFF5B8ABB)
                          : isRec ? const Color(0xFF8CB8F2) : const Color(0xFFD5DAE1),
                      width: isSel || isRec ? 1.8 : 1.0,
                    ),
                  ),
                  alignment: Alignment.center,
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    if (isRec)
                      Text('권장', style: TextStyle(
                          fontSize: 9,
                          color: isSel ? Colors.white70 : const Color(0xFF5B8ABB))),
                  ]),
                ),
              );
            }),
            ...
            children: List.generate(5, (i) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  width: 72, height: 52,
                  decoration: BoxDecoration(
                    color: isSel ? const Color(0xFF5B8ABB) : const Color(0xFFF5F7FA),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                      color: isSel ? const Color(0xFF5B8ABB) : const Color(0xFFD5DAE1),
                      width: isSel ? 1.8 : 1.0,
                    ),
                  ),
                  alignment: Alignment.center,
...
class RoundOptionCard extends StatelessWidget {
  ...
  const RoundOptionCard({
    super.key, required this.type, required this.selected, required this.onToggle,
  });
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: selected ? type.color.withOpacity(0.07) : Colors.white,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(
          color: selected ? type.color : const Color(0xFFD5DAE1),
...
class RoundPreviewRow extends StatelessWidget {
  final int roundNum;
  final RoundType type;
  const RoundPreviewRow({super.key, required this.roundNum, required this.type});
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(ok ? Icons.check_circle_outline : Icons.info_outline,
              size: 16, color: ok ? const Color(0xFF3A8A3A) : const Color(0xFFB08A00)),
          const SizedBox(width: 8),
          Expanded(child: Text(
            ok
...
class GradeSummaryBar extends StatelessWidget {
  ...
  const GradeSummaryBar({super.key, required this.grouped, required this.selectedIds});

  @override
  Widget build(BuildContext context) {
    final grades = ['A', 'B', 'C', 'D', '초심'].where((g) => grouped.containsKey(g)).toList();
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: grades.map((grade) {
          ...
          final sel = grouped[grade]?.where((m) => selectedIds.contains(m.id)).length ?? 0;
          return Column(children: [
            Text('$grade급',
                style: const TextStyle(
                    fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF5B8ABB))),
            const SizedBox(height: 2),
            Text('$sel / $total',
                style: TextStyle(
                    fontSize: 16, fontWeight: FontWeight.w700,
                    color: sel > 0 ? const Color(0xFF111111) : const Color(0xFFCCCCCC))),
          ]);
        }).toList(),
      ),
    );
...
class GradeSectionHeader extends StatelessWidget {
  ...
  const GradeSectionHeader({
    super.key, required this.grade, required this.total, required this.selected,
  });
  ...
  Widget build(BuildContext context) => Row(children: [
    Container(
      ...
      decoration: BoxDecoration(
          color: const Color(0xFF5B8ABB), borderRadius: BorderRadius.circular(8)),
      child: Text('$grade급',
          style: const TextStyle(
              fontSize: 13, fontWeight: FontWeight.w700, color: Colors.white)),
    ),
    const SizedBox(width: 8),
    Text('$selected / $total명 선택',
        style: const TextStyle(
            fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF888888))),
  ]);
}
...
class MemberCheckRow extends StatelessWidget {
  ...
  const MemberCheckRow({
    super.key, required this.member, required this.checked, required this.onChanged,
  });
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      child: Row(children: [
        Checkbox(
          value: checked, onChanged: onChanged,
          materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
          visualDensity: const VisualDensity(horizontal: -3, vertical: -3),
          side: const BorderSide(color: Color(0xFF666D76), width: 1.4),
        ...
        Expanded(child: RichText(
          ...
          text: TextSpan(children: [
            ...
            TextSpan(text: '  ${tournamentAge(member)}세',
                style: const TextStyle(
        ...
        Container(
          ...
          decoration: BoxDecoration(
              color: const Color(0xFFEEF4FB), borderRadius: BorderRadius.circular(8)),
          child: Text(member.grade,
              style: const TextStyle(
                  fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF3F6D98))),
        ),
      ]),
    ),
  );
...
class TournamentPill extends StatelessWidget {
  ...
  const TournamentPill({
    super.key, required this.text, required this.enabled, required this.onTap,
  });
